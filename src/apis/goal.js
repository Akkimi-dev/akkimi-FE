...
export const createSavingGoal = async (payload) => {
  const res = await api.post('/api/v1/saving-goals', payload);
...
export const updateGoal = async (goalId, payload) => {
  const res = await api.patch(`/api/v1/goals/${goalId}`, payload);
  return res.data.result;
};
...
export const deleteGoal = async (goalId) => {
  const res = await api.delete(`/api/v1/goals/${goalId}`);
  return res.data.result;
};
...
export const getCurrentGoals = async () => {
  const res = await api.get('/api/v1/goals/current');
  return res.data.result;
};